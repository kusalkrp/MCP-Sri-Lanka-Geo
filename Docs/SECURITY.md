...
### 2.2 API Key Verification — Two-Layer Auth

Every authenticated request passes through two checks in order:

```
1. .env keys  →  secrets.compare_digest()  — fast, in-memory
2. DB keys    →  SHA-256 hash lookup in api_keys table
```

If either layer accepts the key, the request proceeds. This means `.env` keys (existing consumers) and self-registered DB keys work transparently on the same endpoint.

```python
async def _verify_api_key(provided: str) -> bool:
    if _verify_env_key(provided):   # constant-time compare against .env list
        return True
    return await _verify_db_key(provided)  # SHA-256 lookup in api_keys table
```

All comparisons for `.env` keys use `secrets.compare_digest()` to prevent timing attacks. DB keys use a SHA-256 hash lookup — the raw key is never stored.

### 2.3 API Key Storage — Hashed, Never Plaintext

Self-registered keys are stored as `SHA-256(raw_key)` in the `api_keys` table. The plaintext key is generated with `secrets.token_hex(32)`, returned once to the user, and immediately discarded. It cannot be recovered.

```python
raw_key  = secrets.token_hex(32)       # 64-char hex, never stored
key_hash = hashlib.sha256(raw_key.encode()).hexdigest()
# only key_hash is written to the DB
```

### 2.4 API Key Requirements

- **Minimum length:** 32 characters (64 hex chars for `token_hex(32)`)
- **Format:** Arbitrary string — `secrets.token_hex(32)` is the standard
- **`.env` keys:** Validated at startup by Pydantic — server refuses to start if any key is short
- **DB keys:** Generated by the server at registration — always 64 chars
- **Rotation:** Revoke the old key via `DELETE /admin/keys/{id}`, register a new one — no restart needed

### 2.5 Admin Key

The `ADMIN_KEY` in `.env` controls the `/admin/keys` endpoints (list and revoke). It is checked with `secrets.compare_digest()` and must be at least 32 characters. If `ADMIN_KEY` is empty or short, all admin endpoints return 401.
...
The admin key is separate from API keys — it cannot be used to call MCP tools and API keys cannot be used to access admin endpoints.

### 2.6 stdio Transport — Intentionally No Auth
...
### 2.7 SSE Message Endpoint Security
